...
/// Maximum nesting depth for recursive PDF object parsing (arrays / dictionaries).
///
/// Backport of the RUSTSEC-2026-0187 fix from lopdf 0.42: without a bound, a
/// crafted PDF with deeply nested arrays/dictionaries drives unbounded
/// recursion in the parser and overflows the stack (a DoS that
/// `catch_unwind` cannot catch). The parser threads this counter through
/// `_direct_object`/`array`/`_dictionary`/`inner_dictionary` and returns a
/// `nom::Err::Failure(ErrorKind::TooLarge)` when it hits zero.
