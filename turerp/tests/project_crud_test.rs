...
async fn test_get_project_success() {
    ...
    let create_req = auth_request(actix_web::http::Method::POST, "/api/v1/projects", &token)
        .set_json(json!({
            "tenant_id": 1,
            "name": "Get Test",
            "budget": "5000.00"
...
async fn test_update_project_status_success() {
    ...
    let update_req = auth_request(
        ...
        &format!("/api/v1/projects/{}/status", id),
        &token,
    )
    .set_json(json!({
        "status": "Active"
    ...
    assert_eq!(json["name"], "Status Update Test");
}
...
async fn test_delete_and_restore_project() {
    ...
    let restore_req = auth_request(
        actix_web::http::Method::PUT,
        &format!("/api/v1/projects/{}/restore", id),
        &token,
    )
    .to_request();
...
async fn test_destroy_project_permanently() {
    ...
    let destroy_req = auth_request(
        actix_web::http::Method::DELETE,
        &format!("/api/v1/projects/{}/destroy", id),
        &token,
    )
    .to_request();
