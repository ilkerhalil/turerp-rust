...
impl super::JobRepository for PostgresJobRepository {
    ...
    async fn retry(&self,
        ...
        let result = sqlx::query(
            "UPDATE jobs SET status = 'pending', attempts = 0, last_error = NULL, scheduled_at = NULL, started_at = NULL, completed_at = NULL, updated_at = NOW() WHERE id = $1 AND status = 'failed'",
        )
        .bind(id)
        .execute(&self.pool)
        .await
    ...
    async fn list_schedules(
        ...
        let rows = sqlx::query_as::<_, JobScheduleRow>(
            "SELECT * FROM job_schedules WHERE tenant_id = $1 ORDER BY created_at DESC",
        )
        .bind(tenant_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn update_schedule_next_run(
        ...
        let result = sqlx::query(
            "UPDATE job_schedules SET next_run_at = $1, last_run_at = $2, updated_at = NOW() WHERE id = $3",
        )
        .bind(next_run)
        .bind(last_run)
        .bind(id)
    ...
    async fn toggle_schedule(
        ...
        let result = sqlx::query(
            "UPDATE job_schedules SET is_active = $1, updated_at = NOW() WHERE id = $2",
        )
        .bind(active)
        .bind(id)
        .execute(&self.pool)
    ...
    async fn list_due_schedules(&self,
        ...
        let rows = sqlx::query_as::<_, JobScheduleRow>(
            "SELECT * FROM job_schedules WHERE is_active = true AND (next_run_at IS NULL OR next_run_at <= NOW())",
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|e| map_sqlx_error(e, "JobSchedule"))?;
