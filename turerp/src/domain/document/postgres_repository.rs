...
impl DocumentRepository for PostgresDocumentRepository {
    ...
    async fn create(&self, doc: CreateDocument) -> Result<Document, ApiError> {
        let row = sqlx::query_as::<_, DocumentRow>(
            ...
            RETURNING id, tenant_id, name, filename, size_bytes, mime_type, hash, storage_path, uploaded_by, category_id, tags, description, current_version, created_at, updated_at, deleted_at, deleted_by
            "#,
        )
        .bind(doc.tenant_id)
        .bind(&doc.name)
    ...
    async fn find_by_id(&self, id: i64, tenant_id: i64) -> Result<Option<Document>, ApiError> {
        let row = sqlx::query_as::<_, DocumentRow>(
            r#"
            SELECT id, tenant_id, name, filename, size_bytes, mime_type, hash, storage_path, uploaded_by, category_id, tags, description, current_version, created_at, updated_at, deleted_at, deleted_by FROM documents
    ...
    async fn create_version(
        ...
        let row = sqlx::query_as::<_, DocumentVersionRow>(
            ...
            RETURNING id, document_id, tenant_id, version_number, filename, size_bytes, hash, storage_path, created_by, comment, created_at
            "#,
        )
        .bind(version.document_id)
        .bind(version.tenant_id)
    ...
    async fn list_versions(
        ...
        let rows = sqlx::query_as::<_, DocumentVersionRow>(
            r#"
            SELECT id, document_id, tenant_id, version_number, filename, size_bytes, hash, storage_path, created_by, comment, created_at FROM document_versions
    ...
    async fn create_link(&self, link: CreateDocumentLink) -> Result<DocumentLink, ApiError> {
        let row = sqlx::query_as::<_, DocumentLinkRow>(
            ...
            VALUES ($1, $2, $3, $4, NOW())
            RETURNING id, document_id, tenant_id, entity_type, entity_id, created_at
            "#,
        )
    ...
    async fn find_documents_by_entity(
        ...
        let rows = sqlx::query_as::<_, DocumentRow>(
            r#"
            SELECT d.id, d.d.tenant_id, d.name, d.filename, d.size_bytes, d.mime_type, d.hash, d.storage_path, d.uploaded_by, d.category_id, d.tags, d.description, d.current_version, d.created_at, d.updated_at, d.deleted_at, d.deleted_by FROM documents d
